...
let userPos = null;
let audioCtx, noiseNode, stationNode, gainNode;
let running = false;

// MAP
const map = L.map('map').setView([20, 0], 2);
L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {
  maxZoom: 7, minZoom: 2, attribution: '© OpenStreetMap contributors'
}).addTo(map);

let txMarker = L.marker([transmitter.lat, transmitter.lon]).addTo(map)
  .bindPopup(`Transmitter<br>${transmitter.frequency} MHz`);
let userMarker = null;
...
async function startAudio(){
  ...
  const stationBuffer = await audioCtx.decodeAudioData(arrayBuffer);
